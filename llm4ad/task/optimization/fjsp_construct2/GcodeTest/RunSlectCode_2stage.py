...
def enhanced_local_search(schedule, processing_times, n_jobs, n_machines, max_iterations=1000, time_limit=60):
    ...
    def swap_operations_move(schedule):
        ...
        if random.random() < 0.7:
            machine_id = random.choice(bottleneck_machines)
        else:
            machine_id = random.randrange(n_machines)
        ...
        # 选择交换对 - 可以是非相邻操作
        # 30%概率选择相邻操作，70%概率选择任意两个操作
        if random.random() < 0.3 or len(ops_on_machine) == 2:
            # 选择相邻操作
            idx = random.randrange(len(ops_on_machine) - 1)
            i_idx = idx
            j_idx = idx + 1
        else:
            # 选择任意两个操作
            indices = random.sample(range(len(ops_on_machine)), 2)
            i_idx, j_idx = min(indices), max(indices)
        
        job_i, op_i, start_i, end_i = ops_on_machine[i_idx]
        job_j, op_j, start_j, end_j = ops_on_machine[j_idx]
        ...
        # 检查是否在禁忌列表中
        swap_key = (job_i, op_i, job_j, op_j)
        if swap_key in tabu_list and tabu_list[swap_key] > 0:
            # 允许特赦机制 - 如果是更好的解，仍然接受
            pass
        ...
        # 获取处理时间
        proc_time_i = end_i - start_i
        proc_time_j = end_j - start_j
        
        # 交换操作顺序 - 精细交换
        # 使用插入而非简单的交换，处理可能的依赖关系
        
        # 先移除两个操作
        ops_sequence = [op for op in ops_on_machine if (op[0] != job_i or op[1] != op_i) and (op[0] != job_j or op[1] != op_j)]
        
        # 在i的位置插入j
        j_insertion_idx = i_idx
        ops_sequence.insert(j_insertion_idx, (job_j, op_j, None, None))
        
        # 在j的位置插入i
        i_insertion_idx = j_idx
        if i_insertion_idx <= j_insertion_idx:
            i_insertion_idx += 1
        ops_sequence.insert(i_insertion_idx, (job_i, op_i, None, None))
        
        # 重新计算开始和结束时间
        machine_time = 0
        for idx, (j, o, _, _) in enumerate(ops_sequence):
            if j == job_i and o == op_i:
                # 计算i的新时间
                earliest_start = machine_time
                if op_i > 0:
                    earliest_start = max(earliest_start, neighbor[job_i][op_i-1][2])
                
                end_time = earliest_start + proc_time_i
                neighbor[job_i][op_i] = (machine_id, earliest_start, end_time)
                machine_time = end_time
            
            elif j == job_j and o == op_j:
                # 计算j的新时间
                earliest_start = machine_time
                if op_j > 0:
                    earliest_start = max(earliest_start, neighbor[job_j][op_j-1][2])
                
                end_time = earliest_start + proc_time_j
                neighbor[job_j][op_j] = (machine_id, earliest_start, end_time)
                machine_time = end_time
        
        # 更新两个作业的后续操作
        for next_idx in range(op_i + 1, len(neighbor[job_i])):
            m, s, e = neighbor[job_i][next_idx]
            duration = e - s
            new_s = max(s, neighbor[job_i][next_idx-1][2])
            neighbor[job_i][next_idx] = (m, new_s, new_s + duration)
            
        for next_idx in range(op_j + 1, len(neighbor[job_j])):
            m, s, e = neighbor[job_j][next_idx]
            duration = e - s
            new_s = max(s, neighbor[job_j][next_idx-1][2])
            neighbor[job_j][next_idx] = (m, new_s, new_s + duration)
        
        # 添加到禁忌列表
        tabu_list[swap_key] = tabu_tenure
        ...
        for temp_iter in range(iterations_per_temp):
            ...
            if temp > initial_temp * 0.7:  # 初始阶段 - 更多探索
                move_probs = [
                    ...
                    ('critical_path', 0.25), 
                    ('load_balancing', 0.25),
                    ('execution_time_perturbation', 0.1)  # 给扰动较低的概率
                ]
                ...
                if total_successes > 0:
                    ...
                    move_probs = [
                        ('machine_reassign', 0.25), 
                        ('swap_operations', 0.2), 
                        ('critical_path', 0.25), 
                        ('load_balancing', 0.2),
                        ('execution_time_perturbation', 0.1)
                    ]
            else:  # 后期阶段 - 更多利用
                # 聚焦于历史上最成功的移动类型
